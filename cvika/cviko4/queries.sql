...
SELECT e1.name, d.location, e2.name FROM emp e1 JOIN emp e2 ON e1.deptno = e2.deptno JOIN dept d ON e1.deptno = d.deptno WHERE e1.em>
...
SELECT e.name, d.location, e.salary FROM emp e JOIN dept d ON e.deptno = d.deptno WHERE e.salary > (SELECT min(salary) FROM emp WHER>

/* 13. Which departments contain all job positions? */
SELECT name FROM dept WHERE deptno IN (SELECT deptno FROM emp WHERE deptno NOT IN (SELECT DISTINCT e1.deptno, e2.job FROM emp e1, em>

/* 14. Which departments are empty (have no employees)? */
SELECT deptno FROM dept WHERE deptno NOT IN (SELECT deptno FROM emp);

/* 15. Which employees manage only clerks? */
SELECT name FROM emp WHERE job = 'manager' AND deptno NOT IN (SELECT deptno FROM emp WHERE empno NOT IN (SELECT empno FROM emp WHERE>
