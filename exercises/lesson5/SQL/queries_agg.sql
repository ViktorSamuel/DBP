...
SELECT deptno, job, sum(salary), avg(salary) FROM emp GROUP BY deptno, job;
...
SELECT EXTRACT(YEAR FROM hiredate) AS year, COUNT(*) AS num FROM emp GROUP BY year ORDER BY year;  
...
SELECT EXTRACT(YEAR FROM hiredate) AS year, COUNT(*) AS num FROM emp WHERE year > 1900 GROUP BY year ORDER BY year;  
...
WITH RECURSIVE Subordinates(empno, superior, level) AS (
    SELECT empno, superior, 0
    FROM emp
    WHERE superior IS NULL
    UNION ALL
    SELECT e.empno, e.superior, s.level + 1
    FROM emp e, Subordinates s
    WHERE e.superior = s.empno
)
SELECT empno, level
FROM Subordinates
ORDER BY empno;